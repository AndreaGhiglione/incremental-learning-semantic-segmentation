...
def main(opts):
    ...
    while cur_epoch < opts.epochs and TRAIN:
        ...
        if (cur_epoch + 1) % opts.val_interval == 0:
            ...
            if cur_epoch in list(range(0, 6))+[10, 15, 20, 25]+list(range(30, 40)):
...
if __name__ == '__main__':
    ...
    for param, value in vars(opts).items():
      if param in ['task', 'overlap', 'step', 'batch_size', 'crop_size', 'context_path', 'lr', 'epochs', 'method', 'step_ckpt', 'print_interval', 'sample_num']:
