...
class VOCSegmentationWeak(data.Dataset):
    """`Pascal VOC <http://host.robots.ox.ac.uk/pascal/VOC/>`_ Segmentation Dataset.
    Args:
        root (string): Root directory of the VOC Dataset.
        image_set (string, optional): Select the image_set to use, ``train``, ``trainval`` or ``val``
    ...
    def __init__(self,
        ...
        if is_aug and image_set == 'train':
            ...
            assert os.path.exists(
                mask_dir), "SegmentationClassAug not found"
            split_f = os.path.join(splits_dir, 'train_aug_cues.txt')
        else:
            split_f = os.path.join(splits_dir, image_set.rstrip('\n') + '.txt')
        ...
        # REMOVE FIRST SLASH OTHERWISE THE JOIN WILL start from root
        if image_set == 'train':
            self.images = [(os.path.join(voc_root, x[0][1:]), os.path.join(voc_root, x[1][1:]), x[2]) for x in file_names]
        else:
            self.images = [(os.path.join(voc_root, x[0][1:]), os.path.join(voc_root, x[1][1:])) for x in file_names]

        localization_path = 'data/voc-cues/localization_cues.pickle'
        if image_set == 'train':
            self.cue_data = pickle.load(open(localization_path, 'rb'))
    ...
    def __getitem__(self, index):
        ...
        if self.image_set == 'train':
            label, gt_map = self.get_seed(self.images[index][2])
        ...
        if self.image_set == 'train':
            return img, target, label, gt_map
        else:
            return img, target
    ...
    def get_seed(self, index):
        return self.cue_data["%s_labels" % index], self.cue_data["%s_cues" % index]


class VOCSegmentationIncrementalWeak(data.Dataset):
    def __init__(self,
                 root,
                 train=True,
                 transform=None,
        ...
        full_voc = VOCSegmentationWeak(root, 'train' if train else 'val', is_aug=True, transform=None)
        ...
        if labels is not None:
            ...
            invertedTmp = (tmp_labels, self.inverted_order)

            # make the subset of the dataset
            self.dataset = Subset(full_voc, idxs, transform, target_transform, invertedTmp=invertedTmp)
        else:
            self.dataset = full_voc
