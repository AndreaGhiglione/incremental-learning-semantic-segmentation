...
class Subset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, indices, transform=None, target_transform=None, invertedTmp=None):
        self.dataset = dataset
        self.indices = indices
        self.transform = transform
        self.target_transform = target_transform
        self.invertedTmp = invertedTmp

    def __getitem__(self, idx):
        if self.dataset.image_set == 'train':
            sample, target, label, gt_map = self.dataset[self.indices[idx]]
        else:
        ...
        if self.target_transform is not None:
            ...
            if self.dataset.image_set == 'train':
                for i, val in enumerate(label):
                    val = val.item()
                    label[i] = self.invertedTmp[1][val] if val in self.invertedTmp[0] else 0
                label = label[label > 0]
                
                for i, val in enumerate(gt_map[0]):
                    val = val.item()
                    gt_map[0][i] = self.invertedTmp[1][val] if val in self.invertedTmp[0] else 0

                num_classes = len(self.invertedTmp[1]) - 1
                label_seed = np.zeros((1, 1, num_classes))
                label_seed[0, 0, label] = 1.
                cues = np.zeros([num_classes, 41, 41])
                gt_map = np.array(gt_map).astype(np.int32)
                cues[gt_map[0], gt_map[1], gt_map[2]] = 1.
                label_seed = label_seed.astype(np.float32)
                cues = cues.astype(np.float32)


        if self.dataset.image_set == 'train':
            return sample, target, label_seed, cues
        else:
